...
/* Setup our options */
program
    .option('-i,--input_file <input file>',  'The updates to deploy.  See JSON Format for more information.', checkFileExists)
    .option('-c,--config_file <config file>',  'The JSON configuration file.  See JSON Format for more information.', checkFileExists)
...
if (!program.input_file) { printHelpAndExit("Must set the input file"); }
if (!program.config_file) { printHelpAndExit("Must set the config file"); }
...
var context = new Context(program.config_file);

console.log('inputData: %s', JSON.stringify(context));
...
var client = managementApi.getClient({
    ...
    console.log("Carlos, client:"+JSON.stringify(auth0));
