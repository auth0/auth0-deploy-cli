...
export default class GuardianEmailFactorSettingsHandler extends DefaultHandler {
  existing: Asset;

  constructor(options) {
    super({
      ...options,
      type: 'guardianEmailFactorSettings',
    });
  }
  ...
  async getType(): Promise<Asset | null> {
    ...
    try {
      const data = await this.client.guardian.factors.email.get();
      this.existing = data;
    } catch (err) {
  ...
  async processChanges(assets: Assets): Promise<void> {
    // No API to delete or create guardianEmailFactorSettings, we can only update.
    const { guardianEmailFactorSettings } = assets;

    // Do nothing if not set
    if (!guardianEmailFactorSettings) {
      return;
    }
    ...
    try {
      await this.client.guardian.factors.email.set(
        guardianEmailFactorSettings as Management.SetEmailFactorSettingsRequestContent
      );
    } catch (err) {
    ...
    this.updated += 1;
    this.didUpdate(guardianEmailFactorSettings);
  }
}
